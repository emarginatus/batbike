...
```{r infobord, fig.cap = "Kaart op infoborden", fig.height = 217 / 25.4}
ggplot(origineel, aes(x = long, y = lat, group = group)) +
  geom_polygon(data = water, fill = "#addeff") +
  geom_polygon(data = gras, fill = "#bcde93") +
  geom_polygon(data = park, fill = "#aed1a0") +
  ...
  geom_text(
    data = read_csv("_osm/legende_infopunt.csv"), 
    aes(x = long + 0.001, y = lat - 0.008, label = naam), 
    group = NA, 
    colour = "black",
    size = 10 * mm,
    hjust = 0
  ...
  geom_text(
    data = read_csv("_osm/legende_extra.csv"), 
    aes(label = label, y = lat - 0.008), 
    group = NA, 
    colour = "blue",
    size = 10 * mm,
    hjust = 0
  ) +
  coord_map(xlim = c(4.1951, 4.2616), ylim = c(50.7062, 50.7548))
```
</a>
